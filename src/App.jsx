...
import {
  ...
  ChevronRight, ChevronDown, MoreVertical, Trash2, Pencil, CalendarPlus, Table2, Download,
...
/* 10 cores + Sem cor (cinza) como padrão do comitê */
const TEAMS = {
  sem: { nome: "Sem cor", cor: "#94A3B8" },
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setCarregando(true);
    api.carregarTudo()
      .then((d) => { if (!vivo) return; setPessoas(d.pessoas); setValores(d.valores); setCaixa(d.caixa); setPresencas(d.presencas); setPagamentos(d.pagamentos); })
  ...
  const sair = async () => { try { await api.sair(); } catch (e) { /* segue */ } setLogged(false); };
    ...
    <div style={{ ...FONT, background: G_CANVAS }} className="min-h-screen text-slate-900 flex">
      ...
      <aside className="hidden md:flex w-60 shrink-0 flex-col p-4" style={{ background: G_DARK }}>
        ...
        <nav className="mt-6 space-y-1">
          <NavItem icon={LayoutDashboard} label="Painel" active={page === "dashboard"} onClick={() => setPage("dashboard")} />
          <NavItem icon={Coins} label="Valores" active={page === "valores"} onClick={() => setPage("valores")} />
          <NavItem icon={Wallet} label="Salário" active={page === "salario"} onClick={() => setPage("salario")} />
          <NavItem icon={Users} label="Equipe" active={page === "equipe"} onClick={() => setPage("equipe")} />
          <NavItem icon={CalendarCheck} label="Presença" active={page === "presenca"} onClick={() => setPage("presenca")} />
        </nav>
      ...
      <main className="flex-1 min-w-0 pb-20 md:pb-0 h-screen overflow-y-auto relative flex flex-col">
        <header className="md:hidden sticky top-0 z-10 px-4 py-3 flex items-center justify-between shadow-sm" style={{ background: G_DARK }}>
          <Brand light small />
          <button onClick={sair} className="flex items-center gap-1.5 text-white/90 hover:text-white text-sm font-medium transition-colors">Sair <LogOut size={16} /></button>
        </header>

        <div className="hidden md:flex justify-end p-4 shrink-0">
          <button onClick={sair} className="flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-red-600 transition-colors bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm">Sair do sistema <LogOut size={16} /></button>
        </div>
        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8 flex-1 w-full">
          ...
          {!carregando && page === "dashboard" && <Dashboard pessoas={pessoas} ativos={ativos} valores={valores} caixa={caixa} pagamentos={pagamentos} onEditCaixa={() => setCaixaOpen(true)} />}
          ...
          {!carregando && page === "salario" && <Salario ativos={ativos} pagamentos={pagamentos} onRegistrar={registrarPagamento} onRemover={removerPagamento} pessoas={pessoas} onError={setGlobalError} />}
          {!carregando && page === "equipe" && <Equipe pessoas={pessoas} onValor={abrirValor} onDetail={setDetailFor} onNovo={() => setNovoOpen(true)} />}
          {!carregando && page === "presenca" && <Presenca ativos={ativos} pessoas={pessoas} presencas={presencas} onLancar={lancarPresenca} onRemover={removerPresenca} />}
        </div>
      </main>
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-slate-200 grid grid-cols-5 z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <Tab icon={LayoutDashboard} label="Painel" active={page === "dashboard"} onClick={() => setPage("dashboard")} />
        <Tab icon={Coins} label="Valores" active={page === "valores"} onClick={() => setPage("valores")} />
        <Tab icon={Wallet} label="Salário" active={page === "salario"} onClick={() => setPage("salario")} />
        <Tab icon={Users} label="Equipe" active={page === "equipe"} onClick={() => setPage("equipe")} />
        <Tab icon={CalendarCheck} label="Presença" active={page === "presenca"} onClick={() => setPage("presenca")} />
      </nav>
    ...
    <div style={{ ...FONT, background: `linear-gradient(135deg, ${G_DARK} 0%, ${G} 48%, ${Y} 100%)` }} className="min-h-screen flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-2xl">
          ...
          <input value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-emerald-500 transition-colors" />
          <label className="block text-sm font-medium text-slate-700 mt-4">Senha</label>
          <input type="password" value={senha} onChange={(e) => setSenha(e.target.value)} onKeyDown={(e) => e.key === "Enter" && entrar()} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-emerald-500 transition-colors" />
...
/* ===================== DASHBOARD ===================== */
function Dashboard({ pessoas, ativos, valores, caixa, pagamentos, onEditCaixa }) {
  ...
  return (
    <div>
      <PageTitle title="Painel" sub="Reflete os dados das outras telas. O dinheiro fica oculto até tocar no olho." />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="rounded-2xl border border-slate-200 p-3 sm:p-4 shadow-sm" style={{ background: G_SOFT }}>
          <div className="flex items-center justify-between">
            <div className="text-[10px] sm:text-xs font-semibold uppercase tracking-wide truncate" style={{ color: G_DARK }}>Entrou em caixa</div>
            <div className="flex gap-1 shrink-0">
              <button data-cy="toggle-caixa" onClick={() => setHide((h) => !h)} className="text-slate-500 hover:text-slate-800 transition-colors">{hide ? <EyeOff size={16} /> : <Eye size={16} />}</button>
              <button data-cy="editar-caixa" onClick={onEditCaixa} className="text-slate-500 hover:text-slate-800 transition-colors"><Pencil size={15} /></button>
            </div>
          </div>
          <div className="text-[1.1rem] sm:text-2xl font-extrabold mt-1 truncate" style={{ color: G_DARK }}>{hide ? "R$ ••••••" : brl(entrou)}</div>
          <div className="text-[10px] sm:text-xs text-slate-500 mt-0.5 truncate">verba recebida</div>
        </div>
        <Kpi label="Ativos" value={ativos.length} hint={`${pessoas.length - ativos.length} desligados`} />
        <Kpi label="Sem assinatura" value={semAssin} hint="doc do TSE" />
        <Kpi label="Gasto variável" value={hide ? "R$ ••••" : brl(variavel)} accent={G} />
      </div>
      ...
      <div className="mt-4 grid md:grid-cols-2 gap-4">
        ...
        <Card>
          <h3 className="font-semibold text-slate-800">Gastos fixos vs variáveis</h3>
          <p className="text-sm text-slate-500 mb-3">Quanto já saiu de salário fixo pago e de gasto variável.</p>
          <BarLine label="Salário fixo" val={fixoPago} max={maxG} cor={G} hide={hide} />
          <BarLine label="Variável" val={variavel} max={maxG} cor={AZUL} hide={hide} />
        </Card>
      </div>
    </div>
  );
...
function CaixaModal({ caixa, onAdd, onRemove, onClose }) {
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="flex items-center justify-between mb-3"><h3 className="font-bold text-slate-900">Caixa que entrou</h3><button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors"><X size={20} /></button></div>
      <div className="space-y-2 mb-3 max-h-48 overflow-y-auto pr-1">
        {caixa.map((c) => (
          <div key={c.id} className="flex items-center gap-2 p-2 rounded-lg border border-slate-200 bg-white">
            ...
            <button onClick={() => setExcluirId(c.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors"><Trash2 size={15} /></button>
          </div>
        ))}
        {caixa.length === 0 && <div className="text-center text-sm text-slate-400 py-4">Nenhuma entrada registrada.</div>}
      </div>
      <div className="border-t border-slate-100 pt-3">
        <input value={origem} onChange={(e) => setOrigem(e.target.value)} placeholder="Origem (ex.: Repasse tesouraria)" className="w-full mb-2 rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-emerald-500 transition-colors" />
        <div className="flex gap-2">
          ...
          <button onClick={add} className="rounded-lg px-4 text-white font-semibold transition-opacity hover:opacity-90 shadow-sm shrink-0" style={{ background: G }}>Adicionar</button>
        </div>
      </div>
      {excluirId && (
        <div className="absolute inset-0 bg-white/90 backdrop-blur-sm z-10 flex flex-col items-center justify-center p-6 text-center rounded-2xl">
          ...
          <div className="flex gap-2 w-full">
            <button onClick={() => setExcluirId(null)} className="flex-1 py-2 rounded-lg border border-slate-300 text-slate-600 font-semibold hover:bg-slate-50 transition-colors">Cancelar</button>
            <button onClick={() => { onRemove(excluirId); setExcluirId(null); }} className="flex-1 py-2 rounded-lg bg-red-600 text-white font-semibold hover:bg-red-700 transition-colors">Excluir</button>
          </div>
        </div>
      )}
    </Overlay>
    ...
    <div>
      <div className="flex items-center justify-between mb-4 gap-2">
        ...
        <div className="flex gap-2 shrink-0">
          <button onClick={() => setTabela(true)} className="flex items-center gap-1.5 text-sm font-medium rounded-lg px-3 py-2 border border-slate-300 text-slate-600 bg-white shadow-sm hover:bg-slate-50 transition-colors"><Table2 size={16} /> <span className="hidden sm:inline">Tabela</span></button>
...
function TabelaValores({ valores, pessoas, onBack, onError }) {
  const pes = (id) => pessoas.find((p) => p.id === id) || {};
  const baixar = () => baixarCSV("valores_lancados", ["Nome", "Perfil", "Referente a", "Forma", "Valor"],
    valores.map((v) => [pes(v.personId).nome, pes(v.personId).perfil, v.tipo, FORMA[v.forma].l, money2(v.valor)]), onError);
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-2">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-x-auto" style={{ maxHeight: '70vh' }}>
          <table className="w-full text-sm min-w-[400px]">
            <thead className="sticky top-0 bg-white shadow-sm z-10"><tr className="text-slate-500 border-b border-slate-100">
              <th className="text-left font-medium p-3">Nome</th><th className="text-left font-medium p-3">Referente a</th>
            ...
            <tbody>
              {valores.map((v) => (
                <tr key={v.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  <td className="p-3 text-slate-700 whitespace-nowrap">{pes(v.personId).nome}</td>
...
function Salario({ ativos, pagamentos, onRegistrar, onRemover, pessoas, onError }) {
  ...
  if (nova) return <NovaListaPagamento ativos={ativos} onClose={() => setNova(false)}
        ...
        {pagamentos.map((r) => {
          ...
          return (
            <div key={r.id} className="flex items-center gap-3 p-3">
              <button onClick={() => setDetalhe(r)} className="flex-1 flex items-center gap-3 text-left group min-w-0">
              ...
              <div className="relative shrink-0">
                <button onClick={() => setMenu(menu === r.id ? null : r.id)} className="h-9 w-9 grid place-items-center rounded-lg text-slate-500 hover:bg-slate-50 transition-colors"><MoreVertical size={17} /></button>
                {menu === r.id && <div className="absolute right-0 top-10 z-10 bg-white border border-slate-200 rounded-lg shadow-lg py-1 w-32">
                  <button onClick={() => { setExcluir(r); setMenu(null); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-slate-50 transition-colors"><Trash2 size={15} /> Excluir</button></div>}
              </div>
            </div>
          );
        })}
  ...
  pagamentos.forEach((pag) => {
    pag.itens.forEach((item) => {
      linhas.push({
        ...
        nome: item.nome,
        equipe: TEAMS[item.team]?.nome || "-",
        valor: item.valor,
        pago: item.pago,
      });
    });
  });
  const baixar = () => baixarCSV("historico_salarios", ["Período", "Nome", "Equipe", "Valor", "Status"],
    linhas.map((l) => [l.periodo, l.nome, l.equipe, money2(l.valor), l.pago ? "Pago" : "Não pago"]), onError);
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-x-auto" style={{ maxHeight: '70vh' }}>
          <table className="w-full text-sm min-w-[500px]">
            <thead className="sticky top-0 bg-white shadow-sm z-10"><tr className="text-slate-500 border-b border-slate-100">
              ...
              <th className="text-right font-medium p-3">Valor</th><th className="text-center font-medium p-3">Status</th>
            </tr></thead>
            <tbody>
              {linhas.map((l) => (
                <tr key={l.idUnico} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-3 text-right font-semibold text-slate-800 whitespace-nowrap">{brl(l.valor)}</td>
...
function NovaListaPagamento({ ativos, onClose, onRegistrar }) {
  const [ini, setIni] = useState("2026-09-01"); const [fim, setFim] = useState("2026-09-15");
  ...
  const registrar = () => onRegistrar({ ini, fim, totalPago: jaPago, itens: ativos.map((p) => ({ personId: p.id, nome: p.nome, team: p.team, valor: Number(valorPg[p.id]) || 0, pago: !!pago[p.id] })) });
  return (
    <div>
      <div className="flex items-start justify-between mb-4 gap-2">
        ...
        <div className="flex gap-2 shrink-0">
          <button onClick={onClose} className="text-sm text-slate-600 rounded-lg px-3 py-2 border border-slate-300 hover:bg-slate-50 transition-colors bg-white">Cancelar</button>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3 text-sm bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <input type="date" value={ini} onChange={(e) => setIni(e.target.value)} className="rounded-lg border border-slate-300 px-2 py-1.5 text-sm outline-none focus:border-emerald-500 flex-1 sm:flex-none" />
        <span className="text-slate-400">até</span>
        <input type="date" value={fim} onChange={(e) => setFim(e.target.value)} className="rounded-lg border border-slate-300 px-2 py-1.5 text-sm outline-none focus:border-emerald-500 flex-1 sm:flex-none" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 divide-y divide-slate-100 overflow-hidden shadow-sm">
        {ativos.map((p) => (
          <div key={p.id} className="flex items-center gap-2 sm:gap-3 p-3 hover:bg-slate-50 transition-colors">
            <Avatar p={p} sm />
            <div className="flex-1 min-w-0"><div className="font-medium text-slate-800 truncate">{p.nome}</div><div className="text-[10px] sm:text-xs text-slate-500 truncate">{p.perfil} · {TEAMS[p.team]?.nome}</div></div>
            <div className="flex items-center gap-1 rounded-lg border border-slate-300 px-1 sm:px-2 py-1 bg-white focus-within:border-emerald-500 transition-colors"><span className="text-xs text-slate-400">R$</span>
            ...
            <button data-cy="marcar-pago" onClick={() => setPago((x) => ({ ...x, [p.id]: !x[p.id] }))} className="text-xs sm:text-sm font-semibold rounded-lg px-2 sm:px-3 py-1.5 transition-colors shrink-0" style={pago[p.id] ? { background: G, color: "#fff" } : { background: "#F1F5F9", color: "#475569" }}>{pago[p.id] ? "Pago" : "Pagar"}</button>
          </div>
        ))}
      </div>
      {confirmar && <ConfirmModal title="Registrar pagamento?" msg={`Vai salvar ${ativos.filter((p) => pago[p.id]).length} pago(s), total ${brl(jaPago)}.`} confirmLabel="Registrar" onConfirm={() => { setConfirmar(false); registrar(); }} onClose={() => setConfirmar(false)} />}
    </div>
  );
}
...
function Equipe({ pessoas, onValor, onDetail, onNovo }) {
  const [q, setQ] = useState(""); const [teamF, setTeamF] = useState("todos"); const [perfilF, setPerfilF] = useState("todos"); const [pg, setPg] = useState(0);
  const perPage = 20;
  const filt = pessoas.filter((p) => (teamF === "todos" || p.team === teamF) && (perfilF === "todos" || p.perfil === perfilF)
    && (!q || p.nome.toLowerCase().includes(q.toLowerCase())));
  const sorted = [...filt].sort((a, b) => (a.status === "desligado" ? 1 : 0) - (b.status === "desligado" ? 1 : 0));
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-3">
        <div className="relative flex-1"><Search size={16} className="absolute left-3 top-2.5 text-slate-400" />
          <input value={q} onChange={(e) => { setQ(e.target.value); setPg(0); }} placeholder="Buscar por nome" className="w-full rounded-lg border border-slate-300 pl-9 pr-3 py-2 text-sm outline-none focus:border-emerald-500 transition-colors shadow-sm" /></div>
        <select value={teamF} onChange={(e) => { setTeamF(e.target.value); setPg(0); }} className="rounded-lg border border-slate-300 px-3 py-2 text-sm bg-white focus:border-emerald-500 outline-none transition-colors shadow-sm">
          <option value="todos">Todas as cores</option>{Object.entries(TEAMS).map(([k, t]) => <option key={k} value={k}>{t.nome}</option>)}</select>
        <select value={perfilF} onChange={(e) => { setPerfilF(e.target.value); setPg(0); }} className="rounded-lg border border-slate-300 px-3 py-2 text-sm bg-white focus:border-emerald-500 outline-none transition-colors shadow-sm">
          <option value="todos">Todos os perfis</option>{PERFIS.map((t) => <option key={t} value={t}>{t}</option>)}</select>
      </div>
      <div className="bg-white rounded-2xl border border-slate-200 divide-y divide-slate-100 overflow-hidden shadow-sm">
        {pageItems.map((p) => (
          <div key={p.id} className="flex items-center gap-2 sm:gap-3 p-3 hover:bg-slate-50 transition-colors" data-cy="funcionario-row">
            <Avatar p={p} />
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap"><span className="font-semibold text-slate-800 truncate">{p.nome}</span>
                {p.status === "desligado" && <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-200 text-slate-600">desligado</span>}</div>
            ...
            <button data-cy="funcionario-detalhe" onClick={() => onDetail(p.id)} className="shrink-0 h-9 w-9 grid place-items-center rounded-lg border border-slate-200 text-slate-500 hover:bg-white transition-colors hover:text-slate-800 shadow-sm"><Settings2 size={16} /></button>
          </div>
        ))}
...
/* ===================== PRESENÇA ===================== */
function Presenca({ ativos, pessoas, presencas, onLancar, onRemover }) {
  const [nova, setNova] = useState(false);
  const [tabela, setTabela] = useState(false);
  const [detalhe, setDetalhe] = useState(null);
  const [menu, setMenu] = useState(null);
  ...
  if (nova) return <NovaLista ativos={ativos} datasUsadas={presencas.map((p) => p.data)} onClose={() => setNova(false)}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-2">
        ...
        <div className="flex gap-2 shrink-0">
          <button onClick={() => setTabela(true)} className="flex items-center gap-1.5 text-sm font-medium rounded-lg px-3 py-2 border border-slate-300 text-slate-600 bg-white hover:bg-slate-50 transition-colors shadow-sm"><Table2 size={16} /> <span className="hidden sm:inline">Tabela</span></button>
        ...
        {presencas.map((pr) => {
          ...
          return (
            <div key={pr.id} className="flex items-center gap-3 p-3">
              <button onClick={() => setDetalhe(pr)} className="flex-1 flex items-center gap-3 text-left group min-w-0">
              ...
              <div className="relative shrink-0">
                <button onClick={() => setMenu(menu === pr.id ? null : pr.id)} className="h-9 w-9 grid place-items-center rounded-lg text-slate-500 hover:bg-slate-50 transition-colors"><MoreVertical size={17} /></button>
                {menu === pr.id && (
                  <div className="absolute right-0 top-10 z-10 bg-white border border-slate-200 rounded-lg shadow-lg py-1 w-32">
                    <button onClick={() => { setExcluir(pr); setMenu(null); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-slate-50 transition-colors"><Trash2 size={15} /> Excluir</button>
                  </div>
                )}
              </div>
            </div>
...
function NovaLista({ ativos, datasUsadas, onClose, onLancar }) {
  const [data, setData] = useState("2026-09-08");
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3 bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
        <input type="date" value={data} onChange={(e) => setData(e.target.value)} className="rounded-lg border border-slate-300 px-2 py-1.5 text-sm outline-none focus:border-emerald-500 w-full sm:w-auto" />
        ...
        <button onClick={onClose} className="w-full sm:w-auto mt-2 sm:mt-0 text-sm text-slate-500 underline hover:text-slate-700 transition-colors text-center">Cancelar</button>
      </div>
...
function TabelaGeral({ pessoas, presencas, onBack }) {
  ...
  const baixar = () => baixarCSV("presenca", ["Pessoa", "Status", ...dias.map((d) => fmtData(d.data)), "Total"],
    ordered.map((p) => [p.nome, p.status, ...dias.map((d) => d.marks[p.id] === true ? "Presente" : d.marks[p.id] === false ? "Ausente" : "-"), dias.filter((d) => d.marks[p.id] === true).length]));
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-2">
...
function ValorModal({ pessoas, pessoa, onClose, onSave }) {
  ...
  const matches = pessoas.filter((p) => p.nome.toLowerCase().includes(busca.toLowerCase())).slice(0, 30);

  return (
    <Overlay onClose={onClose}>
      <div className="flex items-center justify-between mb-1"><h3 className="font-bold text-slate-900">Registrar valor dado</h3><button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors"><X size={20} /></button></div>
      ...
      {sel ? (
        ...
        <div className="mt-1 mb-3">
          <div className="relative"><Search size={15} className="absolute left-3 top-2.5 text-slate-400" />
            <input autoFocus value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar funcionário..." className="w-full rounded-lg border border-slate-300 pl-9 pr-3 py-2 text-sm outline-none focus:border-emerald-500 transition-colors shadow-sm" /></div>
          {matches.length > 0 ? (
...
function DetailModal({ pessoa, onClose, upd }) {
  ...
  return (
    <Overlay onClose={onClose} tall>
      <div className="flex items-center gap-3 mb-4"><Avatar p={{ ...pessoa, nome, team }} /><div className="flex-1"><h3 className="font-bold text-slate-900">Dados da pessoa</h3></div><button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors"><X size={20} /></button></div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label={<span className="flex items-center gap-1"><ArrowLeftRight size={14} /> Equipe (cor)</span>}><select value={team} onChange={(e) => setTeam(e.target.value)} className="in bg-white focus:border-emerald-500 transition-colors">{Object.entries(TEAMS).map(([k, t]) => <option key={k} value={k}>{t.nome}</option>)}</select></Field>
      </div>
      ...
      <div className="flex gap-2 mt-2">
        {off ? <button onClick={() => { upd(pessoa.id, { status: "ativo" }); onClose(); }} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-white font-semibold transition-opacity hover:opacity-90 shadow-sm" style={{ background: G }}><UserCheck size={16} /> Reativar</button>
          : <button onClick={() => { upd(pessoa.id, { status: "desligado" }); onClose(); }} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg border border-red-200 text-red-600 font-semibold hover:bg-red-50 transition-colors"><UserMinus size={16} /> Desativar</button>}
        <button data-cy="salvar-pessoa" onClick={() => { upd(pessoa.id, { nome, perfil, team, salario: Number(salario), doc, endereco, exigeAssin, assinou }); onClose(); }} className="flex-1 py-2.5 rounded-lg text-white font-semibold transition-opacity hover:opacity-90 shadow-sm" style={{ background: G_DARK }}>Salvar</button>
      </div>
...
function NovoModal({ onClose, onSave }) {
  ...
  const [endereco, setEndereco] = useState(""); const [entrada, setEntrada] = useState("");
  const docTipo = doc.replace(/\D/g, "").length > 9 ? "CPF" : doc ? "RG" : "";
  const ok = nome && doc && perfil;
  return (
    <Overlay onClose={onClose} tall>
      <div className="flex items-center justify-between mb-4"><h3 className="font-bold text-slate-900">Novo funcionário</h3><button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors"><X size={20} /></button></div>
      <Field label="Nome completo *"><input value={nome} onChange={(e) => setNome(e.target.value)} className="in focus:border-emerald-500 transition-colors" /></Field>
      ...
      <button data-cy="salvar-funcionario" onClick={() => onSave({ nome, doc, endereco, perfil, funcao: perfil, team, exigeAssin: assina, salario: Number(salario) || 0, entrada })} disabled={!ok} className="mt-2 w-full rounded-lg py-2.5 text-white font-semibold disabled:opacity-40 transition-opacity hover:opacity-90 shadow-md" style={{ background: G }}>Cadastrar</button>
      <style>{`.in{margin-top:.25rem;width:100%;border:1px solid #cbd5e1;border-radius:.5rem;padding:.5rem .75rem;font-size:.875rem;outline:none}`}</style>
    </Overlay>
  );
}
...
function Overlay({ children, onClose, tall }) {
  return (<div className="fixed inset-0 z-50 bg-black/40 flex items-end sm:items-center justify-center sm:p-4 backdrop-blur-[2px] transition-opacity" onClick={onClose}>
    <div className={`bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-5 sm:p-6 shadow-2xl relative ${tall ? "max-h-[92vh] overflow-y-auto" : ""}`} onClick={(e) => e.stopPropagation()}>{children}</div></div>);
}
function ConfirmModal({ title, msg, confirmLabel = "Confirmar", cancelLabel = "Sair", danger, onConfirm, onClose }) {
  return (<Overlay onClose={onClose}>
    ...
    <div className="flex gap-2">
      {cancelLabel && <button onClick={onClose} className="flex-1 py-2.5 rounded-lg border border-slate-300 text-slate-600 font-semibold hover:bg-slate-50 transition-colors">{cancelLabel}</button>}
      <button onClick={onConfirm} className="flex-1 py-2.5 rounded-lg text-white font-semibold transition-opacity hover:opacity-90 shadow-sm" style={{ background: danger ? "#DC2626" : G }}>{confirmLabel}</button></div>
  </Overlay>);
}
...
function NavItem({ icon: Ic, label, active, onClick }) { return <button onClick={onClick} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors hover:bg-white/10" style={active ? { background: "rgba(255,255,255,.12)", color: "#fff" } : { color: "#B8D8C2" }}><Ic size={18} /> {label}</button>; }
function Tab({ icon: Ic, label, active, onClick }) { return <button onClick={onClick} className="py-2 flex flex-col items-center justify-center gap-1 text-[10px] font-medium transition-colors hover:bg-slate-50" style={{ color: active ? G : "#94A3B8" }}><Ic size={active ? 22 : 20} className="transition-all duration-300" /> {label}</button>; }
